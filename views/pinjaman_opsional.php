...
<body>
    ...
    <div class="content">
        ...
        <div class="dashboard-header">
            <h2>Pinjaman Buku Opsional</h2>
        </div>
        <div class="container-fluid mt-4">
            <form action="../fungsi/proses_peminjaman_opsional.php" method="POST">
                <div class="mb-3">
...
document.getElementById('kelas').addEventListener('change', function() {
    var bukuSelects = document.querySelectorAll('.buku-input');
    bukuSelects.forEach(bukuSelect => {
        bukuSelect.innerHTML = '<option selected disabled>Pilih Buku</option>';
    
        // Fetch buku yang berkategori selain "pelajaran"
        fetch(`../fungsi/get_buku_opsional.php`)
            .then(response => response.json())
            .then(data => {
                if (data.length > 0) {
                    data.forEach(buku => {
...
document.getElementById('buku-container').addEventListener('change', function(event) {
    if (event.target.classList.contains('buku-input')) {
        ...
        if (selectElements.length > 0 && selectElements[0].value !== '' && bukuGroups.length < 10) {
            ...
            // Fetch buku yang berkategori selain "pelajaran" for the new select element
            fetch(`../fungsi/get_buku_opsional.php`)
                .then(response => response.json())
                .then(data => {
                    if (data.length > 0) {
